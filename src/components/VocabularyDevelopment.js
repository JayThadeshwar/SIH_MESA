...
const VocabularyDevelopment = () => {
  ...
  const fetchVocab = async () => {
    ...
    const renderData = data.map((item, key) => (
      <>
        <div
          className={cx("accordion accordion", styles.acc)}
        ...
        >
          <div className="accordion-item mt-3">
            <h2 className="accordion-header" id={"flush-heading" + key}>
              <button
                className={cx(
                  "accordion-button collapsed text-capitalize",
                  styles.accBtn
                )}
                type="button"
                        ...
                        {socketio !== undefined && (
                          <span
                            onClick={() => {
                              ...
                              let payload =
                                voiceApiFunction.voicePostFunction();
  ...
  useEffect(() => {
    ...
    fetchData();
    return () => {
      if (socketio !== undefined) socketio.close();
    };
  }, []);
    ...
    <div className={cx("VocabDev", styles.vocabDev)}>
      ...
      <div className={cx("container-fluid p-5", styles.topVocabDev)}>
        ...
        <div className={cx(styles.chapDetail, "")}>
          {isChapterLoading && clickableArr.length !== 0 ? (
            <LoadingSpinner />
          ) : (
            clickableArr.map((item) => {
              return (
                <>
                  {" "}
                  {socketio !== undefined && (
